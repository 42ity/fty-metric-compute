/*  =========================================================================
    fty_metric_compute_selftest.c - run selftests
...
// Now built only with --enable-drafts, so even stable builds are hidden behind the flag
    { "cmstats", NULL, true, false, "cmstats_test" },
    { "cmsteps", NULL, true, false, "cmsteps_test" },
    { "private_classes", NULL, false, false, "$ALL" }, // compat option for older projects
#endif // FTY_METRIC_COMPUTE_BUILD_DRAFT_API
// Tests for stable public classes:
    { "fty_mc_server", fty_mc_server_test, true, true, NULL },
...
{
    test_item_t *item;
    printf ("Running fty-metric-compute selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_METRIC_COMPUTE_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_metric_compute_private_selftest (verbose, item->subtest);
#endif // FTY_METRIC_COMPUTE_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_metric_compute_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-metric-compute test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_METRIC_COMPUTE_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_metric_compute_private_selftest (verbose, test->subtest);
#endif // FTY_METRIC_COMPUTE_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
